...
# Applied Effective R in Animal breeding - Summer 2021 Project
...
## Step 1 : Data extraction and initialization

Package check if exist it loads it, if not exist it will install the missing required and recommended package. The following packages are needed directly or indirectly to run this program.
...
For the function to work or to start to work the user need to do two tasks. First, load the necessary packages and compile the function by running it. Second, invoke or call the function by typing do_plot() directly to the console, R Script, R Markdown or R Notebook code chunk. These two processes will take on average around 3 minutes.

When called the program will guide the user at every step of action giving them a simple choices to choose, and the user need to confirm their choice by typing the provided keys such as 1 or 2.

Following is how the function is called, after loading the packages and compiling it.
...
The following while loop check if user enters the correct file and file path It make sure the program exit without crushing. It will exit if the user don't have the Rdata or if the user just want to exit. The main thing it does other than checking the user input, is Loading the Rdata object from a Mobps run and saving it to variable pop.
...
Using Mobps package get.cohorts extracting cohorts and saving them in the variable cohorts
...
Using Mobps package population\$info\$trait.name extracting the traits the number of traits could be one or more than one
...
Initialization of empty vectors TBV, Cohorts, TimeP for data frame construction
...
Loop to extract TBV, Cohorts and TimeP and saving them to their respective empty vectors
...
Forming of data frame nemed Fdat by joining the vectors TBV, Cohorts, TimeP The data frame Fdat saves the data without separating the traits
...
Extracting the cohorts with repeats from mobps naming of repeats grabbing "\_" and saving to Cohort_Rept
...
The following loop will Extract what is common to the repeated Cohorts name or from Cohort_Rept and save to vector Cohort_Rept_Com
...
The following loop will grab or extract what is unique that is the number after "\_" as unique identifier between repeats of same class
...
Creating data frame that contain the above three vectors to save the unique naming and id of the repeats
...
The following will subset Fdat extracting all the columns and rows that have cohorts repeats and save it to temporary data frame or TempRepDat
...
The following will bind the columns of TempRepDat (Which contain repeats cohorts TBV among other three columns) and Repeats_DF (which contain the uniqu cohort naming and Id of the repeats) then create new data frame combining both RepDat
...
The following will subset Fdat extracting all the columns and rows that not have cohorts repeats and save it to NonRepDat data frame
...
The following three lines add three new columns to facilitate for the next rbind operation
...
The following will rbind the None repeats and the Repeats TBV data in to one master data called Fdat
...
## Step 2: Traits separation and saving respective data to a data frame in a list

Initializing empty list() to save the separated traits value of TBV, Cohorts, and TimeP
...
The for loop to initialize based on the number of traits empty data frames for each traits with columns TBV, Cohorts, TimeP, cohort with repeats, cohorts repeats id, cohort repeats common class.
...
The for loop to fill up the created empty data frames with in the listTraits list with separated rows of each respective traits from the initial data frame Fdat row values of TBV, Cohorts, TimeP to their respective data frame within the listTraits
...
## Step 3: Calculation of Average Breeding Value and saving in a data frame to a list

Initializing empty list AverageBVList to save data frames of Average BV, Cohorts and TimeP for each traits

```{r}
  AverageBVList <- list()
```

For loopInitializing empty data frames within the list AverageBVList to save in the future the separated AverageBV, Cohorts, TimeP based on each traits
...
loop to calculate average of BV for cohorts and saving them for each trait in AverageBVList list in the desired column order
...
This Step will create a uniqueCohortsDF which contain all the cohorts classes which are common for the cohort repeats.
...
## Step 4: Dialogue based user interface to control desired output

This is the longest chunk of the code which its aim is to receive the user choice such as simple numbers 1 or 2 or 3 or 4 to out put one of the four output of the function from Step 5 to Step 7 below.

## Step 5: Data visualization using ggplots employing the interactive functionality of plotly package
...
## Step 6: Options to save data to computer
...
## Step 7: Option to get an HTML summary
...
The function output the desired result successfully while catching errors, but depending on the size and construction of the RData, the execution time could be longer and in addition it could affect the successful production of the desired output such as the plots and HTML summary.

The function was tested to see if it will do its intended tasks using six different RData produced by MOBPS and were downloaded from mobps.de website. The result of the test showed the function works as intended, but could be slower if the RData is larger, or if the number of traits used in the simulated breeding program are many.

## References

Torsten Pook, Martin Schlather, Henner Simianer, MoBPS - Modular Breeding Program Simulator, *G3 Genes\|Genomes\|Genetics*, Volume 10, Issue 6, 1 June 2020, Pages 1915--1918, <https://doi.org/10.1534/g3.120.401193>

Pook, T., Schlather, M., & Simianer, H. (2020, June 11). Modular Breeding Program Simulator. *MoBPS v1.6.31* . Goettingen: University of Goettingen. Retrieved from <https://www.mobps.de/>

<https://stackoverflow.com/questions/32064744/loading-a-rdata-file-inside-a-function>
